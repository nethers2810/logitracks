...
router = APIRouter(prefix="/orders", dependencies=[Depends(require_roles("admin", "planner", "analyst"))])
...
def create_order(payload: OrderCreate, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
...
def update_order(order_id: int, payload: OrderUpdate, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
    return order_service.update(db, order_id, payload.model_dump(exclude_unset=True))


@router.delete("/{order_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_order(order_id: int, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
...
def create_order_item(order_id: int, payload: OrderItemCreate, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
...
def update_order_item(order_id: int, order_item_id: int, payload: OrderItemUpdate, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
...
def delete_order_item(order_id: int, order_item_id: int, db: Session = Depends(get_db), _=Depends(require_roles("admin", "planner"))):
    existing = db.get(OrderItem, order_item_id)
    if not existing or existing.order_id != order_id:
        raise AppError("Order item not found in order", status_code=404, code="not_found")

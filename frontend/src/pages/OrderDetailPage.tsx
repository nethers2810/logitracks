...
export function OrderDetailPage() {
  ...
  const runSimulation = async () => {
    if (!orderId) return;
    const res = await api.post<{ run_id: number }>(`/orders/${orderId}/simulate`);
    await refetch();
  ...
  return <QueryBlock isLoading={isLoading} isError={isError} isEmpty={!data}><div className="space-y-4">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2">
        {data!.header.latest_run_id ? <Link className="rounded bg-brand-600 px-3 py-2 text-sm text-white" to={`/simulation/${data!.header.latest_run_id}`}>View latest simulation</Link> : null}
        <button className="rounded border border-brand-600 px-3 py-2 text-sm text-brand-700" onClick={runSimulation}>Run simulation</button>
      </div>
    </div>
